...
export default function Home() {
    ...
    const [heightDiv, setHeightDiv] = useState('62%');
    ...
    const handleScrollFiltro = (event) => {
        ...
        if ((scrollTop + clientHeight + 2) >= scrollHeight) {
            if(consultandoTickesPrincipal == true && terminoDeCargarTickets == true){
                ...
                if(menuActivo == 1){
                    if(submenuActivo == 1){
                        cargarTicketsPendientes(usuarioId,(offsetActivo+10))
                    }
                    if(submenuActivo == 2){
                        cargarTicketsTrabajando(usuarioId,(offsetActivo+10))
                    }
                }
                else if(menuActivo == 2){
                    cargarTicketsPagos(usuarioId,(offsetActivo+10))
                }
                else if(menuActivo == 3){
                    cargarTicketsProduccion(usuarioId,(offsetActivo+10))
                }
                else if(menuActivo == 4){
                    cargarTicketsDespachados(usuarioId,(offsetActivo+10))
                }
                else if(menuActivo == 5){
                    cargarTicketsClientes(usuarioId,(offsetActivo+10))
                }
                else if(menuActivo == 6){
                    cargarTicketsNoInteresados(usuarioId,(offsetActivo+10))
                }
            }
    ...
    useEffect(() => {
        if(loadingSite == 0){
            ...
            if(token != undefined){
                ...
                console.log(process.env.ENDPOINT_API+"/static/"+decodedToken.payload.foto);
                ...
                cargarTicketsTrabajando(decodedToken.payload.user,0);
    ...
    */
    ...
    async function blobToBase64(blob) {
        ...
        reader.onload = async () => {
            ...
            await enviarAudioEnFragmentos(ticketActivo.id, base64String, usuario, hora);
            
            //envia el mensaje en base64 al backend para que lo envie
            /*await axios.post(process.env.ENDPOINT_API+'/whatsapp2/enviarAudio',
            { 
    ...
    */
    ...
    const handleScroll = (event) => {
        ...
        if (div.scrollTop === 0) {
            if(banderaConsultandoMessages == false && tieneMensajesPendientes == true){
                ...
                idMessage = mensajesCargadosTicket[mensajesCargadosTicket.length-1].id;
    ...
    function abrirTicket(datos){
        ...
        banderaConsultandoMessages = true;
        if(appMovil){
            setdivStyleHeight('calc(100% - 90px)')
    ...
    */
    async function cargarContadoresProcesos(id){
        await axios.post(process.env.ENDPOINT_API+'/whatsapp2/ticketContadorProce',
            {asesor: id}
        ).then(data=>{
            if(usuarioId == data.data.asesor){
                var opcionesMenuCopi = opcionesMenu.map(menu => {
                    ...
                    if (menu.id === 1) {
                        for (var i = data.data.inbox.length - 1; i >= 0; i--) {
                            if(subMenu.length > 0){
                                for (var e = subMenu.length - 1; e >= 0; e--) {
                                    if(subMenu[e].id == data.data.inbox[i].idStatus){
                                        subMenu[e].tickets += 1
                                        if(subMenu[e].subFiltros.length > 0){
                                            for (var h = subMenu[e].subFiltros.length - 1; h >= 0; h--) {
                                                if(subMenu[e].subFiltros[h].id == 0){
                                                    subMenu[e].subFiltros[h].contador += 1;
                                                }else{
                                                    if(data.data.inbox[i].processesid == subMenu[e].subFiltros[h].id){
                                                        subMenu[e].subFiltros[h].contador += 1;
                                                    }
                                                }
                                            }
                            ...
                            if(data.data.inbox[i].unreadMessages > 0){
                                if(subMenu.length > 0){
                                    for (var e = subMenu.length - 1; e >= 0; e--) {
                                        if(subMenu[e].id == data.data.inbox[i].idStatus){
                                            subMenu[e].messages += data.data.inbox[i].unreadMessages;
                                            subMenu[e].conversaciones += 1;
                                        }
                                    }
                                }
                                var banderaChat = true;
                                messagesSinLeer += data.data.inbox[i].unreadMessages;
                                conversacionesConMsg += 1;
                            }
                        }
                        //subMenu
                        return {
                            ...menu,
                            tickets: data.data.inbox.length,
                    ...
                    // Pagos
                        ...
                        for (var i = data.data.pago.length - 1; i >= 0; i--) {
                            if(data.data.pago[i].unreadMessages > 0){
                                var banderaChat = true;
                                messagesSinLeer += data.data.pago[i].unreadMessages;
                                conversacionesConMsg += 1;
                            }
                        }
                        return {
                            ...menu,
                            tickets: data.data.pago.length,
                            conversaciones: conversacionesConMsg,
                            messages: messagesSinLeer
                        };
                    }
                    // En Produccion
                        ...
                        for (var i = data.data.produccion.length - 1; i >= 0; i--) {
                            if(data.data.produccion[i].unreadMessages > 0){
                                var banderaChat = true;
                                messagesSinLeer += data.data.produccion[i].unreadMessages;
                                conversacionesConMsg += 1;
                            }
                        }
                        return {
                            ...menu,
                            tickets: data.data.produccion.length,
                            conversaciones: conversacionesConMsg,
                            messages: messagesSinLeer
                        };
                    }
                    // Despachado
                        ...
                        for (var i = data.data.despachados.length - 1; i >= 0; i--) {
                            if(data.data.despachados[i].unreadMessages > 0){
                                var banderaChat = true;
                                messagesSinLeer += data.data.despachados[i].unreadMessages;
                                conversacionesConMsg += 1;
                            }
                        }
                        return {
                            ...menu,
                            tickets: data.data.despachados.length,
                            conversaciones: conversacionesConMsg,
                            messages: messagesSinLeer
                        };
                    }
                    // Clientes
                        ...
                        for (var i = data.data.clientes.length - 1; i >= 0; i--) {
                            if(data.data.clientes[i].unreadMessages > 0){
                                var banderaChat = true;
                                messagesSinLeer += data.data.clientes[i].unreadMessages;
                                conversacionesConMsg += 1;
                            }
                        }
                        return {
                            ...menu,
                            tickets: data.data.clientes.length,
                            conversaciones: conversacionesConMsg,
                            messages: messagesSinLeer
                        };
                    }
                    // No Interesados
                        ...
                        for (var i = data.data.noInteresados.length - 1; i >= 0; i--) {
                            if(data.data.noInteresados[i].unreadMessages > 0){
                                var banderaChat = true;
                                messagesSinLeer += data.data.noInteresados[i].unreadMessages;
                                conversacionesConMsg += 1;
                            }
                        }
                        return {
                            ...menu,
                            tickets: data.data.noInteresados.length,
                            conversaciones: conversacionesConMsg,
                            messages: messagesSinLeer
                        };
                    }else{
    ...
    */
    ...
    const cargarMessagesTicket = async (ticket, offset) => {
        ...
        try {
            const response = await axios.post(`${process.env.ENDPOINT_API}/whatsapp2/cargarMessagesTicket`, {
                ticket: ticket,
                offset: offset
            });
            ...
            if (ticket === response.data.ticket) {
                ...
                if (response.data.messages.length === 0 && offset === 0) {
                    setContenidosMessagesChat([
                        <div key="no-messages" style={{ width: '100%', textAlign: 'center' }}>
                ...
                response.data.messages.reverse().forEach(message => {
                    ...
                    if (!multimedia && message.mediaKey && message.mediaKey !== '0' && message.mediaKey !== 'undefined') {
                        if (!colaMultimedia.some(media => media.mediaKey === message.mediaKey)) {
                ...
                for(var i = 0; i < arrcolaEnviandoMessage.length; i++){
                    //arrcolaEnviandoMessage[i].ticket == ticket && parseInt(arrcolaEnviandoMessage[i].hora) > message.created
                ...
                setMessagesChat(prevMessages => [...response.data.messages, ...prevMessages]);
    ...
    const renderMessage = (message, multimedia) => {
        ...
        return (
            <li className={`message ${clase}`} key={message.id} id={`message-${message.id}`}>
                ...
                {message.quotedMsg !== 0 && message.mencionado && message.mencionado.length > 0 && (
                    <div className={`mencionado-${clase}`}>
                        {message.mencionado[0].mediaKey && message.mencionado[0].mediaKey !== '0' && message.mencionado[0].mediaKey !== 'undefined' ? (
                            <div className={`multimedia-${message.mencionado[0].mediaKey}`}>
                                <p>
                            ...
                            <p style={{ textAlign: 'left' }}>{message.mencionado[0].body}</p>
                        )}
                    </div>
                )}
    ...
    const cargarMultimedia = async () => {
        console.log(colaMultimedia.length);
        if (colaMultimedia.length > 0) {
    ...
    */
    async function cargarTicketsTrabajando(id,offset){
        offsetActivo = offset;
        await axios.post(process.env.ENDPOINT_API+'/whatsapp2/ticketTrabajando',{
            asesor: id,
            offset:offset,
            ...
            if(ticket.length == 0 || ticket.length < 50){
            ...
            if(searchBuscador.length > 0 && data.data.ticketGeneral.length > 0){
    ...
    */
    async function cargarTicketsPendientes(id,offset){
        ...
        await axios.post(process.env.ENDPOINT_API+'/whatsapp2/ticketPendientes',{
            ...
            if(ticketTra.length == 0 || ticketTra.length < 50){
                terminoDeCargarTickets = false
            };
    ...
    */
    ...
    return (
        <>  
            ...
            <div className="opacityOpen" style={{height:appMovil === true ? '100%':'114%'}}>
                <div className="card" style={{height:'100%'}}>
                    <div className="card-body" style={{height:'100%'}}>
                        <div className="container testimonial-group divChat2" style={{padding:'0px'}}>
                            <div className="row flex-nowrap overflow-auto">
                                {opcionesMenu.map((menu) => (
                                    <div 
                                        key={menu.id}
                                        className={`col-6 col-sm-2 ${menuSeleccionado === menu.id ? 'menu-activo' : ''}`} 
                                        onClick={() => cambioMenuPrincipal(menu.id)} 
                                        style={{ 
                                            borderRight: menu.borderRight === true ? '3px solid #b7b7b7' : 'none', 
                                            cursor: 'pointer',
                                            textAlign: 'center',
                                    ...
                                    >
                                        <div style={{ padding: '2px 5px' }}>
                                            <span style={{ textAlign: 'left', width: '100%', display: 'block', fontSize: '12px' }}>
                                                {menu.name}: {menu.tickets} Conv.
                                            </span>
                                            ...
                                            {menu.conversaciones > 0 ? (
                                                <span 
                                                    style={{ 
                                                        background: menu.alertMessage === 1 ? '#279b0a' : 'red',
                                                        color: 'white',
                                                        borderRadius: '20px',
                                                        padding: '3px 10px',
                                                        fontSize: '12px'
                                                ...
                                                >
                                                    {menu.messages} Mess. - {menu.conversaciones} Conv.
                                                </span>
                                            ) : null}
                                        </div>
                                    </div>
                        ...
                        <div className='row' style={{height:'calc(100% - 50px)'}}>
                            <div className="col-12 col-sm-4 divChat2"  style={{border: '1px solid #b7b7b7',maxHeight: '94%',marginTop: '10px'}}>
                                <React.Fragment>
                                {opcionesMenu.map((menu) => (
                                ...
                                <div className='row'>
                                    <div className='col-12' style={{marginTop:'20px'}}>
                                        <input type="text" className="form-control buscador" onChange={(event) => handleSearch(event.target.value)} placeholder="Buscar" aria-label="Username" aria-describedby="basic-addon1"/>
                                    </div>
                                    {opcionesMenu.map((menu) => (
                                ...
                                <div className="row" style={{maxHeight: heightDiv,overflow:'auto',marginTop:'5px'}} onScroll={handleScrollFiltro}>
                                    {consultandoTickets ? (
                                            ...
                                            {ticketsConsulta.map((ticket) => (
                                                ticket != undefined && (
                                                    <div className='row' key={'ticketPendiente-'+ticket.id} onClick={(event) => abrirTicket(ticket)} style={{cursor:'pointer',width:'98%',margin:'auto auto 10px',borderLeft: `2px solid ${ticket.border}`,background: ticket.background,padding: '5px',paddingBottom:'0px'}} key={ticket.id}>
                                                        ...
                                                        <div className="col-4" style={{display: appMovil ? 'none':'block',fontSize:'small',textAlign:'left',borderTop:`1px solid ${ticket.border}`,padding:'5px 0px'}}>
                                                            "{ticket.pushname}"
                                                        </div>
                                        ...
                                        ticketsConsultaGeneral.map((ticket) => (
                                            <div className='row' key={'ticketPendiente-'+ticket.id} onClick={(event) => abrirTicket(ticket)} style={{cursor:'pointer',width:'98%',margin:'auto auto 10px',borderLeft: `2px solid ${ticket.border}`,background: ticket.background,padding: '5px',paddingBottom:'0px'}} key={ticket.id}>
                                                    ...
                                                    <div className='col-6' style={{padding:'0px',textAlign:'right'}}>
                                                        <span onClick={(event) => espiarChat(ticket)} style={{background: '#ffb93a',color: '#000',border: '2px solid #ffb93a',borderRadius: '20px',padding: '3px 5px 3px 3px',fontSize: '12px',cursor:'pointer',marginRight:'5px'}}>
                            ...
                            <div className="col-12 col-sm-8" id='divChat' style={{display:appMovil === true ? 'none':'block', padding:'0px',border: '1px solid #b7b7b7',maxHeight: '94%',marginTop: '10px'}}>
                                {ticketAbierto ? (
                                    <>
                                    ...
                                    <div style={{height:divStyleHeight}}>
                                        ...
                                        <div className='row' style={{ display: chatActivo ? 'block' : 'none',position:'relative',background:'#e6e6e6',width:'100%',margin:'auto',height:'150px'}}>
                                            ...
                                            <div className='col-12' style={{paddingTop:'3px',paddingRight:'3px',paddingLeft:'3px'}}>
                                                <div style={{textAlign:'left',marginTop: '0px',background: '#d3d3d3',height: '113px',position: 'absolute',width: '100%',overflow: 'hidden',zIndex: '10',padding: '10px 5px',display:showDropdown ? 'block' : 'none'}}>
                                                    ...
                                                    <div style={{textAlign: 'right',marginTop: '3px'}}>
                                                        <div className='row'>
                                                            <div className='col-12 col-sm-4' style={{display:'flex'}}>
                                                            {recording && (
                                                                <>
                                                                <i
