...
export default function Home() {
    ...
    const handleFechaDesdeChange2 = (event) => {
        ...
        setFechaDesde2(event.target.value);
        setConsultando2(true);
        cargarData2();
    };

    const handleFechaHastaChange2 = (event) => {
        fecha4 = event.target.value;
        setFechaHasta2(event.target.value);
        setConsultando2(true);
        cargarData2();
    }
    ...
    useEffect(() => {
        ...
        setFechaHasta2(obtenerFechaActual());
        cargarData();
        cargarData2();
    }, []);
    ...
    async function cargarData(){
        await axios.post(process.env.ENDPOINT_API+'/whatsapp/reporteTitckets',{ 
            ...
            for (var e = arrayAsesores.length - 1; e >= 0; e--) {
                //venta
                arrayAsesores[e].venta = 0;
                for (var i = response.data.pedidos.length - 1; i >= 0; i--) {
                    if(response.data.pedidos[i].usuario == arrayAsesores[e].id){
                        arrayAsesores[e].venta += parseInt(response.data.pedidos[i].valor);
                    }
                }
    ...
    async function cargarData2(){
        await axios.post(process.env.ENDPOINT_API+'/anuncios/resultVentas',{ 
            hasta: fecha4,
            desde: fecha3
        }).then(response => {
                    ...
                    <div className="card-body">
                        <div className="d-flex align-items-start align-items-sm-center gap-4">
                            <div className='row' style={{width:'100%'}}>
                                    ...
                                    <table className="table table-striped">
                                        <thead>
                                            <tr>
                                                ...
                                                <th colspan="4" style={{background:'#566a7f',color:'white',textAlign:'center'}} scope="col">Aceptados</th>
                                            </tr>
                                            <tr>
                                                ...
                                                <th style={{background:'#889eb5',color:'white',textAlign:'center'}} scope="col">Resplandor</th>
                                                <th style={{background:'#889eb5',color:'white',textAlign:'center',borderRight:'4px solid #566a7f',borderLeft:'4px solid rgb(86, 106, 127)'}} scope="col">TOTAL</th>
                                                <th style={{background:'#889eb5',color:'white',textAlign:'center',borderRight:'4px solid #566a7f'}} scope="col">VENTAS</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {consultando ? (
                                                <tr>
                                                    <td style={{textAlign:'center'}} colspan="8">Consultando...</td>
                                                </tr>
                                            ):(
                                ...
                                <div className='col-12'>
                                    <h1 style={{textAlign:'center',marginTop:'40px',marginBottom:'40px'}}>REPORTE CAMPAÑAS</h1>
                                </div>
                                <div className='col-6'>
                                    <label>Desde:</label>
                                ...
                                <div className='col-6'>
                                    <div className="form-group">
                                        <label>Hasta:</label>
                                        <input type="date" onChange={handleFechaHastaChange2} className="form-control" placeholder="Hasta" value={fechaHasta2}></input>
                                    </div>
                                </div>
                                <div className='col-12' style={{marginTop:'20px'}}>
                                    <table className="table table-striped">
                                        <thead>
                                            <tr>
                                                <th style={{background:'#889eb5',color:'white',textAlign:'center'}} scope="col">CAMPAÑA</th>
                                                <th style={{background:'#889eb5',color:'white',textAlign:'center'}} scope="col">RESULTADO</th>
                                                <th style={{background:'#889eb5',color:'white',textAlign:'center'}} scope="col">CONVERSACIONES</th>
                                                <th style={{background:'#889eb5',color:'white',textAlign:'center'}} scope="col">PEDIDOS</th>
                                                <th style={{background:'#889eb5',color:'white',textAlign:'center',borderRight:'4px solid #566a7f',borderLeft:'4px solid rgb(86, 106, 127)'}} scope="col">VENTA</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {consultando2 ? (
                                                ...
                                                <>
                                                    {datos2.map((dato2, index) => (
                                                        <tr key={index}>
                                                            ...
                                                            <td style={{textAlign:'center'}}>
                                                                {dato2.contactos.length}
                                                            </td>
                                                            <td style={{textAlign:'center'}}>
                                                                {dato2.contPedidos}
                                                            </td>
                                                            <td className='total' style={{textAlign:'center',borderRight:'4px solid #566a7f',borderLeft:'4px solid rgb(86, 106, 127)',background:'#889eb5',color:'white',backgroundColor:'rgb(136, 158, 181) !important'}}>
                                                                {formattedPrice(dato2.pedidos)}
                                                            </td>
                                                        </tr>
                                                    ))}

                                                    <tr>
                                                        <td style={{background:'#889eb5',color:'white',textAlign:'center', fontWeight: 'bold'}}>TOTAL</td>
                                                        <td style={{textAlign:'center', fontWeight: 'bold'}}>
                                                            {datos2.reduce((total, dato2) => total + dato2.resultado, 0)}
                                                        </td>
                                                        <td style={{textAlign:'center', fontWeight: 'bold'}}>
                                                            {datos2.reduce((total, dato2) => total + dato2.contactos.length, 0)}
                                                        </td>
                                                        <td style={{textAlign:'center', fontWeight: 'bold'}}>
                                                            {datos2.reduce((total, dato2) => total + dato2.contPedidos, 0)}
                                                        </td>
                                                        <td style={{textAlign:'center', fontWeight: 'bold', borderRight:'4px solid #566a7f',borderLeft:'4px solid rgb(86, 106, 127)',background:'#889eb5',color:'white'}}>
                                                            {formattedPrice(datos2.reduce((total, dato2) => total + dato2.pedidos, 0))}
                                                        </td>
                                                    </tr>
                                                </>
                                            )}
