...
export default function Home() {
    ...
    async function pedidosFacturacion(){
        await axios.post(process.env.ENDPOINT_API+'/pedidos/dataNotas').then(response => {  
    ...
    async function facturar() {
        if(pedidosFacturacionData.length == 0){
            ...
            withReactContent(Swal).fire({
                type: "info",
                title: 'Alerta',
                text: "Elige uno o varios pedidos para hacer las notas",
            });
        }else{
            ...
            swalWithReactFactura.fire({
                ...
                html: (
                    <div>
                        <p>
                            Ingresa el consecutivo de SIIGO (J-2)
                        </p>
        ...
        for (var i = 0; i <  pedidosFacturacionData.length; i++) {
            ...
            for(var e = 0; e < pedidosData.length; e++){
                if(pedidosData[e].referencia == pedidosFacturacionData[i]){
                    ...
                    documentoCruce = pedidosData[e].documentoCruce
                    asesor = pedidosData[e].name
                    itemCruce = pedidosData[e].itemCruce;
                    descuentoTotal = parseInt(pedidosData[e].descuento);
                    excedenteTotal = parseInt(pedidosData[e].excedente);
                    fechaFactura = pedidosData[e].fechaFactura;
                    e = pedidosData.length
                }
            }
            console.log(documentoCruce);
            if(descuentoTotal > 0){
                ...
                if(productosDataFinPedido[e].pedido == pedidosFacturacionData[i] && productosDataFinPedido[e].cobrado > 0){
                    ...
                    for(var h = 0; h < cuentasData.length; h++){
                        if(cuentasData[h].linea == productosDataFinPedido[e].linea && cuentasData[h].grupo == productosDataFinPedido[e].grupo){
                            cuenta = cuentasData[h]['cuentas devoluciones'];
                    ...
                    data.push({ 
                        'TIPO DE COMPROBANTE (OBLIGATORIO)': 'J', 
                        ...
                        'CÓDIGO DEL MOTIVO DE DEVOLUCIÓN': "2",
                        'FORMA DE PAGO': "0",
                        ...
                        'FACTURA ELECTRÓNICA A DEBITAR/ACREDITAR': "F-002",
                        'NÚMERO DE FACTURA ELECTRÓNICA A DEBITAR/ACREDITAR': documentoCruce,
            ...
            if(saldoTotal > 0){
                ...
                const [fechaPart, _] = fechaFactura.split(" ");
                ...
                data.push({ 
                    ...
                    'TIPO DE MONEDA ELABORACIÓN': "0",
                    'TIPO Y COMPROBANTE CRUCE': "F-002",
                    'NÚMERO DE DOCUMENTO CRUCE': documentoCruce,
                    'NÚMERO DE VENCIMIENTO': itemCruce,
                    'AÑO VENCIMIENTO DE DOCUMENTO CRUCE': year,
                    'MES VENCIMIENTO DE DOCUMENTO CRUCE': month,
                    'DÍA VENCIMIENTO DE DOCUMENTO CRUCE': day,
                    'DESCRIPCIÓN DE COMENTARIOS': "Ped: "+pedidosFacturacionData[i]+" Ase:"+asesor
                })
                contadorSecuencia += 1;
            }
    ...
    function descargarXLSFactura(){
        let fecha2 = new Date();
        saveAs(blob, 'NOTASDEBIDOS_J2_'+fecha2.getDate()+"_"+(fecha2.getMonth() +1)+"_"+fecha2.getFullYear()+"__"+fecha2.getHours()+":"+fecha2.getMinutes()+":"+fecha2.getSeconds()+'.xlsx');
    }
    ...
    function reportarFacturacion(){
        ...
        swalWithReact.fire({
            title: "Confirmación",
            html: "¿Esta segur@ de reportar las notas?",
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                await axios.post(process.env.ENDPOINT_API+'/pedidos/aprobacionNotas',{ 
                    usuario: usuarioId,
                    pedidos: dataResumen,
                        ...
                        <div style={{display: 'flex',alignItems: 'center',height:'100%'}}>
                            <div className='row' style={{width:'100%',margin:'auto'}}>
                                <div className='col-12' style={{textAlign:'center'}}>
                                    <button className='btn btn-secondary'  onClick={() => descargarXLSFactura()} style={{width:'100%',borderRadius: '0px',margin:'10px'}}><i style={{marginRight:'5px'}} className='bx bxs-cloud-download'></i> Notas Credito - {contFacturas}</button>
                                </div>
                        ...
                        <div className="d-flex align-items-start align-items-sm-center gap-4">
                            <div className='row' style={{width:'100%',marginTop:'20px'}}>
                                <div className='col-12' style={{textAlign:'right'}}>
                                    <button onClick={() => facturar()} className='btn btn-primary' style={{borderRadius: '0px'}}>Realizar</button>
                                </div>
                                    ...
                                    <table className="table table-striped">
                                        <thead>
                                            <tr style={{background:"#e9e9e9"}}>
                                                ...
                                                <th scope="col" style={{textAlign:'center'}}>Factura</th>
                                                        ...
                                                        return (
                                                            <React.Fragment key={index}>
                                                                <tr>
                                                                    ...
                                                                    <td style={{ textAlign: 'center' }}>F-2 {pedido.documentoCruce}</td>
