    ...
    async function pedidosCartera(){
        await axios.post(process.env.ENDPOINT_API+'/pedidos/dataCartera').then(response => {  
            if(response.data.bandera == 0){
                ...
                tercerosData = response.data.terceros.concat(response.data.tercerosRecaudo);
                //RECAUDOS TCC
                /*for (var i = 0; i < response.data.recaudosTCC.length; i++) {
                    response.data.recaudosTCC[i].pedidos = [{
                        pedido: response.data.recaudosTCC[i].pedido,
                        valor: response.data.recaudosTCC[i].recaudo,
                        tipo: 1,
                        factura:response.data.recaudosTCC[i].factura,
                        fechaFactura:response.data.recaudosTCC[i].fechaFactura,
                        item: response.data.recaudosTCC[i].item
                    }];
                    response.data.recaudosTCC[i].tipo = 1;
                    response.data.recaudosTCC[i].transportadora = 1;
                    response.data.recaudosTCC[i].valor = response.data.recaudosTCC[i].recaudo;
                    pedidosData.push(response.data.recaudosTCC[i]);
                }

                //RECAUDOS SKY
                for (var i = 0; i < response.data.recaudosSky.length; i++) {
                    response.data.recaudosSky[i].pedidos = [{
                        pedido: response.data.recaudosSky[i].pedido,
                        valor: response.data.recaudosSky[i].recaudo,
                        tipo: 1,
                        factura:response.data.recaudosSky[i].factura,
                        fechaFactura:response.data.recaudosSky[i].fechaFactura,
                        item: response.data.recaudosSky[i].item
                    }];
                    response.data.recaudosSky[i].tipo = 1;
                    response.data.recaudosSky[i].transportadora = 2;
                    response.data.recaudosSky[i].valor = response.data.recaudosSky[i].recaudo;
                    pedidosData.push(response.data.recaudosSky[i]);
                }*/

                //mensajero
                for (var i = 0; i < response.data.mensajero.length; i++) {
                    response.data.mensajero[i].pedidos = [{
                        pedido: response.data.mensajero[i].pedido,
                        valor: response.data.mensajero[i].recaudo,
                        tipo: 1,
                        factura:response.data.mensajero[i].factura,
                        fechaFactura:response.data.mensajero[i].fechaFactura,
                        item: response.data.mensajero[i].item
                    }];
                    response.data.mensajero[i].tipo = 1;
                    response.data.mensajero[i].transportadora = 4;
                    response.data.mensajero[i].valor = response.data.mensajero[i].recaudo;
                    pedidosData.push(response.data.mensajero[i]);
                }
    ...
    function prepararExcelFacturacion(){
        ...
        for (var i = 0; i <  pedidosData.length; i++) {
            ...
            for (var h = 0; h <  pedidosData[i].pedidos.length; h++) {
                ...
                let descripcion = "CANCELA FACTURA - ";
                if(pedidosData[i].pedidos[h].tipo == 1){
                    descripcion += "RECAUDO -";
                }
                descripcion += pedidosData[i].pedidos[h].pedido+" - ASE:"+pedidosData[i].name;
                ...
                data.push({ 
                    ...
                    'NIT': Nit,
                    'SUCURSAL': "0",
                    'DESCRIPCIÓN DE LA SECUENCIA': descripcion,
                    'NÚMERO DE CHEQUE': "0",
            ...
            if(pedidosData[i].tipo == 1){
                //es un recaudo
                if(pedidosData[i].transportadora == 1){
                    //recaudo de TCC
                    data.push({ 
                        'TIPO DE COMPROBANTE (OBLIGATORIO)': 'R', 
                        'CÓDIGO COMPROBANTE  (OBLIGATORIO)': '1', 
                        ...
                        'NIT': Nit,
                        'SUCURSAL': "0",
                        'DESCRIPCIÓN DE LA SECUENCIA': "BANCO TCC",
                        'NÚMERO DE CHEQUE': "0",
                        'COMPROBANTE ANULADO': "N",
                        'CÓDIGO DEL MOTIVO DE DEVOLUCIÓN': "0",
                        'FORMA DE PAGO': "0",
                ...
                if(pedidosData[i].transportadora == 2){
                    //recaudo de SKY
                    data.push({ 
                        'TIPO DE COMPROBANTE (OBLIGATORIO)': 'R', 
                        'CÓDIGO COMPROBANTE  (OBLIGATORIO)': '1', 
                        'NÚMERO DE DOCUMENTO': consecutivoFactura,
                        'CUENTA CONTABLE   (OBLIGATORIO)': "1120150400",
                        'DÉBITO O CRÉDITO (OBLIGATORIO)': "D",
                        'VALOR DE LA SECUENCIA   (OBLIGATORIO)': (pedidosData[i].valor),
                        'AÑO DEL DOCUMENTO': fecha.getFullYear(),
                        'MES DEL DOCUMENTO': (fecha.getMonth() +1),
                        ...
                        'NIT': Nit,
                        'SUCURSAL': "0",
                        'DESCRIPCIÓN DE LA SECUENCIA': "BANCO SKYDROPX",
                        'NÚMERO DE CHEQUE': "0",
                        'COMPROBANTE ANULADO': "N",
                        'CÓDIGO DEL MOTIVO DE DEVOLUCIÓN': "0",
                        'FORMA DE PAGO': "0",
                ...
                if(pedidosData[i].transportadora == 4){
                    //recaudo de mensajero
                    data.push({ 
                        'TIPO DE COMPROBANTE (OBLIGATORIO)': 'R', 
                        'CÓDIGO COMPROBANTE  (OBLIGATORIO)': '1', 
                        'NÚMERO DE DOCUMENTO': consecutivoFactura,
                        'CUENTA CONTABLE   (OBLIGATORIO)': "1105050100",
                        'DÉBITO O CRÉDITO (OBLIGATORIO)': "D",
                        'VALOR DE LA SECUENCIA   (OBLIGATORIO)': (pedidosData[i].valor),
                        'AÑO DEL DOCUMENTO': fecha.getFullYear(),
                        'MES DEL DOCUMENTO': (fecha.getMonth() +1),
                        ...
                        'NIT': Nit,
                        'SUCURSAL': "0",
                        'DESCRIPCIÓN DE LA SECUENCIA': "CAJA",
                        'NÚMERO DE CHEQUE': "0",
                        'COMPROBANTE ANULADO': "N",
                        'CÓDIGO DEL MOTIVO DE DEVOLUCIÓN': "0",
                        'FORMA DE PAGO': "0",
