...
export default function Home() {
    ...
    async function cargarPedidos(){
        ...
        await axios.post(process.env.ENDPOINT_API+'/apoyoTSU/Despachos2',{ 
    ...
    async function despacharPedido(datos,id){
        console.log($("#transportadora-"+id).val());
        //tcc
        if($("#transportadora-"+id).val() == 1){
            ...
            await axios.post(process.env.ENDPOINT_API+'/apoyoTSU/guardarRemesaTCC2',{ 
        ...
        if($("#transportadora-"+id).val() == 3){
            ...
            await axios.post(process.env.ENDPOINT_API+'/apoyoTSU/guardarRemesaInter2',{ 
                referencia:datos.referencia,
                usuario: usuarioId
            }).then(response => {
            ...
            swalWithReact.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    await axios.post(process.env.ENDPOINT_API+'/apoyoTSU/guardarRemesa2',{ 
                        referencia:datos.referencia,
                        guia: numeroDeGuia,
                        usuario: usuarioId,
    ...
    function rotuloPedido(datos){
        ...
        $("#destinatarioRotulo").html(datos.nombre.toUpperCase()+" ("+datos.documento+")");
        $("#direccionRotulo").html(datos.direccion.toUpperCase()+" "+datos.observacion.toUpperCase());
        $("#ciudadRotulo").html(datos.departamento.toUpperCase()+' - '+datos.ciudad.toUpperCase());
        $("#telefonoRotulo").html('TELEFONO: '+datos.telefono.toUpperCase());
        $(".divMensajero").hide();
        if($("#transportadora-"+datos.id).val() == 4){
            $(".divMensajero").show();
        }
        $("#pedidoRotulo").html('PED:'+datos.referencia.toUpperCase()+" "+datos.observacion.toUpperCase());
                        ...
                        <div className='row' style={{marginTop:'10px'}}>
                            ...
                            <div className='col-9'>
                                <h3 style={{lineHeight:'60px',width:'100%'}}>
                                    <span style={{borderBottom:'2px dotted #333',width:'100%',display:'block'}}>TRES SON UNO S.A.S - 901.449.779</span>
                                                        ...
                                                        <tr key={index}>
                                                            ...
                                                            <td style={{textAlign:'center'}}>
                                                                ...
                                                                {result.saldo > 0 && (
                                                                    <>
                                                                        <br></br>
                                                                        <b style={{color:'red'}}>{formattedPrice(result.saldo)}</b>
                                                                    </>
                                                                )}
                                                            ...
                                                            <td style={{textAlign:'center'}}>
                                                                {result.tipoEnvio != 4 && (
                                                                    <>
                                                                        {result.ciudad}<br></br>{result.departamento}
                                                                    </>
                                                                )}
                                                            </td>
                                                            <td style={{textAlign:'center'}}>
                                                                ...
                                                                <div style={{borderLeft:'1px solid #333',background:'#e9e9e9',padding:'0px 5px',textAlign:'left'}}>
                                                                    Tel: {result.telefono}
                                                                    <br></br>
                                                            ...
                                                            <td style={{textAlign:'center'}}>
                                                                {result.tipoEnvio != 4 && (
                                                                    <select onChange={(e) => handleChangeTransportadora(index, e)} value={result.transportadora} id={`transportadora-${result.id}`} className='form-control'>
                                                                        ...
                                                                        <option value="3">Inter Home</option>
