...
export default function Home() {
        ...
        reader.onload = (e) => {
            ...
            data.map((row) => {
                ...
                if(row['persona'] != undefined){
                    ...
                    const original = row['factura'];

                    const partes = original.split("-");
                    const letra = partes[0];                    // 'F'
                    const parte1 = parseInt(partes[1], 10);     // 2
                    const parte2 = parseInt(partes[2], 10);     // 16442
                    const parte3 = parseInt(partes[3], 10);     // 2

                    const resultado = `${letra} ${parte1}-${parte2}`;
                    ...
                    filteredData.push({
                        ...
                        persona: row['persona'],
                        factura: resultado,
                        saldoSiigo: row['saldo'],
                        facturaIntera: parte2,
                        itemFactura: parte3,
    ...
    function decodeData(data) {
        ...
        for (const key in data) {
            if (key === 'NÃºmero de guÃ­a') {
                decodedData['guia'] = data[key];
            } else if(key === "Valor guÃ­a"){
                decodedData['valorGuia'] = data[key];
            } else {
    ...
    async function cotejarRelacion(datos){
        await axios.post(process.env.ENDPOINT_API+'/TCC/cotejarCartera',{ 
            pedidos: datos
        }).then(response => { 
            if(response.data.bandera == 1){
                ...
                response.data.pedidos.map((row) => {
                    ...
                    if(row['validando'] != 0){
                        pedidosConfi.push(row);
                    }
                    
                })
                setResumenRelacion(dataResult);
                setData(pedidosConfi)
            }else{
    ...
    async function cargarResult() {
        await axios.post(process.env.ENDPOINT_API+'/TCC/resultdespachosSky')
        .then(response => { 
            /*if(response.data.bandera == 1){
                ...
                <div className='row'>
                    <div className='col-12'>
                        <div className='row' style={{marginBottom:'10px'}}>
                            <label>Archivo:</label>
                            <input type="file" onChange={handleFileUpload} accept=".xlsb, .xlsx, .xls, .csv" />
                        </div>
                            ...
                            <table className="table">
                                <thead>
                                    <tr style={{background:"#e9e9e9"}}>
                                        ...
                                        <th scope="col" style={{textAlign:'center'}}>tercero</th>
                                        <th scope="col" style={{textAlign:'center'}}>factura</th>
                                        
                                        <th scope="col" style={{textAlign:'center'}}>saldo</th>
                                        <th scope="col" style={{textAlign:'center'}}>notas</th>
                                        <th scope="col" style={{textAlign:'center'}}>pedido</th>
                                        <th scope="col" style={{textAlign:'center'}}>asesora</th>
                                    ...
                                    return (
                                        <React.Fragment key={index}>
                                            <tr style={{background:color}} key={index}>
                                                ...
                                                <td style={{textAlign:'center'}}>{row.persona}</td>
                                                <td style={{textAlign:'center'}}>{row.factura}</td>
                                                
                                                <td style={{textAlign:'center'}}>{formattedPrice(row.saldoSiigo)}</td>
                                                <td style={{textAlign:'center'}}>{row.nota}</td>
                                                <td style={{textAlign:'center'}}>{row.pedido}</td>
                                                <td style={{textAlign:'center'}}>{row.asesora}</td>
                                                <td style={{textAlign:'center'}}>{row.cliente}</td>
                                                <td style={{textAlign:'center'}}>{row.guia}</td>
                                            </tr>
                                        </React.Fragment>
                                    );
                                })}
            ...
            <div className='row' style={{height:'100%',width:'98%',margin:'auto'}}>
                            ...
                            <div className="d-flex align-items-start align-items-sm-center">
                                <div className='row' style={{width:'100%',margin:'auto',marginTop:'20px'}}>
                                    <div className='col-12' style={{marginBottom:'20px',textAlign:'right'}}>
                                        <button onClick={() => abrirRelacionPago()} className='btn btn-primary'>Relacion .csv</button>
                                    </div>
                ...
                <div className='col-3' style={{maxHeight:'100vh',overflow:'auto',background:'#f5f5f5',paddingTop:'40px',paddingBottom:'40px',boxShadow:'0px 0px 18px 0px #bdbdbdbf'}}>
                    <div style={{borderLeft:'3px solid #a3a3a3',background:'#e9e9e9',padding:'10px'}}>
                        <span style={{cursor:'pointer'}}>Pendiente: <b>{formattedPrice(resultRecaudo.pendiente)}</b></span>
                        <br></br>
                        <span style={{cursor:'pointer'}}>Disponible: <b>{formattedPrice(resultRecaudo.disponible)}</b></span>
