...
export default function Home() {
    ...
    const handleFileUpload = (event) => {
        ...
        reader.onload = (e) => {
            ...
            data.map((row) => {
                row = decodeData(row);
                if(row['N. de rastreo'] != undefined && row['Estado'] != 'Cancelado'){
                    let recaudo = 0;
                    if(parseInt(row['Precio']) > 0){
                        recaudo = parseInt(row['Precio'])
                    }
                    filteredData.push({
                        contador: contador,
                        remesa: row['N. de rastreo'], 
                        pedido: 'Validando...', 
                        recaudo: recaudo,
                        destinatario: row['Destinatario'],
                        estadoSky: row['Estado'],
                        validando: 0,
                        nota: ''
                    });
                    contador += 1;
    ...
    async function cotejarRelacion(datos){
        await axios.post(process.env.ENDPOINT_API+'/TCC/cotejarRelacionSky',{ 
            pedidos: datos
        }).then(response => { 
                            ...
                            <table className="table">
                                <thead>
                                    <tr style={{background:"#e9e9e9"}}>
                                        ...
                                        <th scope="col" style={{textAlign:'center'}}>Recaudo</th>
                                    ...
                                    return (
                                        <React.Fragment key={index}>
                                            <tr style={{background:color}} key={index}>
                                                ...
                                                <td style={{textAlign:'center'}}>{row. destinatario}</td>
                                                <td style={{textAlign:'center'}}>{formattedPrice(row.recaudo)}</td>
