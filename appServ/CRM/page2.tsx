...
export default function Home() {
    ...
    type formTicket = {id: string; name: string;number: string; image: string;nameLabel: string,unreadMessages: number,timestamp:string,bodyMessage:string,timestampMessage:string,ackMessage:string,ultimoMessageDate: string,ackIcon:string,numberLabel:string};
    ...
    const divStyle = {
        ...
        width: '100%',
        height: chatActivo ? 'calc(100% - 13.3em)' : ticketAbierto ? 'calc(100% - 50px)' : '100%',
        ...
        if(tokenLoading == 0){
            ...
            const fetchData = async (usuarioId) => {
                ...
                socket.on('ackMessageActualizado', (data)=>{
                    ...
                    var updatedTicketTrabajando2 = updatedTicketTrabajando.map(ticketLocal => {
                        // Verificar si el ID del ticket es igual a 1
                        if (ticketLocal.id === data.valida.ticket && ticketLocal.timestampMessage <= data.valida.timestampMessage) {
            ...
            if(token != undefined){
                ...
                setUsuario({
                    ...
                    foto   : process.env.ENDPOINT_API+decodedToken.payload.foto,
    ...
    */
            ...
            if(ticket == response.data.ticket && ticketAbierto2.id == response.data.ticket){
                if(offset == 0 && response.data.messages.length == 0){
                    ...
                    for (var i = response.data.messages.length - 1; i >= 0; i--) {
                        ...
                        if(response.data.messages[i].multimedia === 1){
                            let banderaAgrgearMultimedia = true;
                            for (var e = colaMultimedia.length - 1; e >= 0; e--) {
                        ...
                        const nuevoSpan = (
                            <li className={`message ${clase}`} id={`message-${response.data.messages[i].id}`}>
                                <img title={response.data.messages[i].titleAsesor} className={`logo logo${clase}`} src={fotomessage} alt="" />
                                    {response.data.messages[i].multimedia === 1 ? (
                                        <>
                                        {response.data.messages[i].typeMultimedia == 3 ? ( 
                                            ...
                                            <>
                                                <div style={{height:'35vh'}} className={`multimedia-${response.data.messages[i].mediaKey}`}>
                                                    <p>
                                                        <i style={{marginTop:'-3px'}} className='bx bx-loader bx-spin'></i>
                                                        Cargando Multimedia
                                                    </p>
        ...
        await axios.post(process.env.ENDPOINT_API+'/whatsapp/cargarMultimediaMessage',{ 
            ...
            if(response.data.message != undefined){
                ...
                if (multimediaElements.length > 0) {
                    ...
                    if(response.data.message.typeWhatsapp == 2 || response.data.message.typeWhatsapp == 6){
                    ...
                    //es una nota de voz
                    if(response.data.message.typeWhatsapp == 3 || response.data.message.typeWhatsapp == 7){
                        // Itera sobre todos los elementos multimedia encontrados
                        Array.from(multimediaElements).forEach(multimediaElement => {
    ...
    async function enviarMensajeArchivo(){
        if(file == null){
            ...
            const response = await axios.post(process.env.ENDPOINT_API+'/whatsapp/upload', formData, {
                headers: {
    ...
    return (
        <>
                    ...
                    <div className='row'>
                        {detallMultimedia && (
                            <>
                                ...
                                <div className='col-6' style={{borderRight:'2px solid #222d32'}}>
                                    <p>
                    ...
                    <div className="card mb-4">
                        <div className="card-body">
                            <div className="row">
                                <div className="col-12">
                                    <div className="row">
                                        <div className={`col ${menuSeleccionado === 1 ? 'menu-activo' : ''}`} onClick={() => cambioMenuPrincipal(1)} style={{textAlign:'center',borderRight: '3px solid #b7b7b7',padding:'2px 5px',cursor:'pointer'}}>
                                            ...
                                            {messagesInbox.messages > 0 ? (
                                                <span style={{background: messagesInbox.alert === 1 ? '#279b0a' : 'red',color: 'white',borderRadius: '20px',padding: '3px 10px',fontSize: '12px'}}>{messagesInbox.messages} Mess. - {messagesInbox.conversaciones} Conv.</span>
                                            ):(null)}
                                        </div>
                                        <div className={`col ${menuSeleccionado === 2 ? 'menu-activo' : ''}`} onClick={() => cambioMenuPrincipal(2)} style={{textAlign:'center',borderRight: '3px solid #b7b7b7',cursor:'pointer',padding:'2px 5px'}}>
                                            ...
                                            {messagesPagos.messages > 0 ? (
                                                <span style={{background: messagesPagos.alert === 1 ? '#279b0a' : 'red',color: 'white',borderRadius: '20px',padding: '3px 10px',fontSize: '12px'}}>{messagesPagos.messages} Mess. - {messagesPagos.conversaciones} Conv.</span>
                                            ):(null)}
                                        </div>
                                        <div className={`col ${menuSeleccionado === 3 ? 'menu-activo' : ''}`} onClick={() => cambioMenuPrincipal(3)} style={{textAlign:'center',borderRight: '3px solid #b7b7b7',cursor:'pointer',padding:'2px 5px'}}>
                                            ...
                                            {messagesProduccion.messages > 0 ? (
                                                <span style={{background: messagesProduccion.alert === 1 ? '#279b0a' : 'red',color: 'white',borderRadius: '20px',padding: '3px 10px',fontSize: '12px'}}>{messagesProduccion.messages} Mess. - {messagesProduccion.conversaciones} Conv.</span>
                                            ):(null)}
                                        </div>
                                        <div className={`col ${menuSeleccionado === 4 ? 'menu-activo' : ''}`} onClick={() => cambioMenuPrincipal(4)} style={{textAlign:'center',borderRight: '3px solid #b7b7b7',cursor:'pointer',padding:'2px 5px'}}>
                                            ...
                                            {messagesDespacho.messages > 0 ? (
                                                <span style={{background: messagesDespacho.alert === 1 ? '#279b0a' : 'red',color: 'white',borderRadius: '20px',padding: '3px 10px',fontSize: '12px'}}>{messagesDespacho.messages} Mess. - {messagesDespacho.conversaciones} Conv.</span>
                                            ):(null)}
                                        </div>
                                        <div className={`col ${menuSeleccionado === 5 ? 'menu-activo' : ''}`} onClick={() => cambioMenuPrincipal(5)} style={{textAlign:'center',borderRight: '3px solid #b7b7b7',cursor:'pointer',padding:'2px 5px'}}>
                                            ...
                                            {messagesClientes.messages > 0 ? (
                                                <span style={{background: messagesClientes.alert === 1 ? '#279b0a' : 'red',color: 'white',borderRadius: '20px',padding: '3px 10px',fontSize: '12px'}}>{messagesClientes.messages} Mess. - {messagesClientes.conversaciones} Conv.</span>
                                            ):(null)}
                                        </div>
                                        <div className={`col ${menuSeleccionado === 6 ? 'menu-activo' : ''}`} onClick={() => cambioMenuPrincipal(6)} style={{textAlign:'center',cursor:'pointer',padding:'2px 5px'}}>
                                            ...
                                            {messagesNoInteresados.messages > 0 ? (
                                                <span style={{background: messagesNoInteresados.alert === 1 ? '#279b0a' : 'red',color: 'white',borderRadius: '20px',padding: '3px 10px',fontSize: '12px'}}>{messagesNoInteresados.messages} Mess. - {messagesNoInteresados.conversaciones} Conv.</span>
                                            ):(null)}
                                        </div>
                                    </div>
                                </div>
                                <div className="col-4" style={{border: '1px solid #b7b7b7',height: 'calc(100vh - 202px)',maxHeight: 'calc(100vh - 202px)',marginTop: '10px'}}>
                                    <div className={`${menuSeleccionado === 1 ? 'div-activo' : ''}`} style={{display:'none'}}>
                                        ...
                                        <div className="row" style={{maxHeight: '93%'}}>
                                            ...
                                            <div className={`col ${menuSeleccionado1 === 1 ? 'div-activoOpciones' : ''}`} style={{textAlign:'center',marginTop:'10px',marginBottom:'10px',display:'none'}}>
                                                <span onClick={(event) => filtroTrabajando(1)} style={{cursor:'pointer',margin:'0px 5px',background: selectFiltro == 1 ? '#e1e1e1' : 'white',color: '#333',border:'2px solid #e1e1e1',borderRadius: '20px',padding: '3px 10px',fontSize: '12px'}}>Todos: {conTodosFiltro}</span>
                                                <span onClick={(event) => filtroTrabajando(2)} style={{cursor:'pointer',margin:'0px 5px',background: selectFiltro == 2 ? '#e1e1e1' : 'white',color: '#333',border:'2px solid #d1bb6d',borderRadius: '20px',padding: '3px 10px',fontSize: '12px'}}>Interesados: {conInteresadosFiltro}</span>
                                                <span onClick={(event) => filtroTrabajando(3)} style={{cursor:'pointer',margin:'0px 5px',background: selectFiltro == 3 ? '#e1e1e1' : 'white',color: '#333',border:'2px solid #92bf92',borderRadius: '20px',padding: '3px 10px',fontSize: '12px'}}>Pendiente pago: {conPendientesPagoFiltro}</span>
                                                    ...
                                                    <div className='col-6'>
                                                        ...
                                                        <div className="form-check" style={{width:'fit-content'}}>
                                                            ...
                                                            <label className="form-check-label" style={{textAlign:'left'}}>
                                                                Mensajes sin Respuesta (Cliente)
                                                            </label>
                                                        </div>
                                                    </div>
                                                    <div className='col-6' style={{textAlign:'right',paddingTop:'15px'}}>
                                                        <span style={{cursor:'pointer'}} onClick={(event) => filtroFecha(1)}>Fecha 
                                                            {seleFecha1 ? (
                                                                <>
                                                                    <span style={{marginLeft:'4px'}}>Ascendente</span> 
                                                                    <i style={{marginTop:'-4px',color:'red',transform:'rotate(90deg)'}} className='bx bx-transfer-alt'></i>
                                                                </>
                                                            ):(
                                                                <>
                                                                    <span style={{marginLeft:'4px'}}>Descendientes</span>
                                            ...
                                            <div className='col-12' style={{padding:'0px',marginTop:'8px'}}>
                                                <div className={`${menuSeleccionado1 === 1 ? 'div-activoOpciones2' : ''}`} onScroll={handleScrollFiltro(1)} style={{display:'none',height: '51vh',maxHeight: '51vh',overflow:'auto'}}>
                                                ...
                                                <div className={`${menuSeleccionado1 === 2 ? 'div-activoOpciones2' : ''}`} onScroll={handleScrollFiltro(1)} style={{display:'none',height: '63vh',maxHeight: '63vh',overflow:'auto'}}>
                                                            ...
                                                            {ticketPendientes && ticketPendientes.length > 0 ? (
                                                                ticketPendientes.map((dato2) => (
                                                                    <div className='row' style={{width:'98%',borderLeft: '2px solid #b7b7b7',background: '#adadad4d',margin: 'auto',marginBottom:'10px',padding: '5px',paddingBottom:'0px'}} key={dato2.id}>
                                                                        ...
                                                                        <div className='col-8' id={`optionTicketCola-${dato2.id}`} style={{textAlign:'right',borderTop:'1px solid #b7b7b7',padding:'5px 0px'}}>
                                    ...
                                    <div className={`${menuSeleccionado === 2 ? 'div-activo' : ''}`} style={{display:'none'}}>
                                        ...
                                        <div className="row" style={{maxHeight: '93%'}}>
                                            ...
                                            <div className='col-6' style={{textAlign:'right',paddingTop:'15px'}}>
                                                <span style={{cursor:'pointer'}} onClick={(event) => filtroFecha(2)}>Fecha 
                                                    {seleFecha2 ? (
                                                        <>
                                                            <span style={{marginLeft:'4px'}}>Ascendente</span> 
                                                            <i style={{marginTop:'-4px',color:'red',transform:'rotate(90deg)'}} className='bx bx-transfer-alt'></i>
                                            ...
                                            <div className='col-12' onScroll={handleScrollFiltro(2)} style={{maxHeight: '63vh',overflow:'auto',padding:'0px 7px'}}>
                                    ...
                                    <div className={`${menuSeleccionado === 3 ? 'div-activo' : ''}`} style={{display:'none'}}>
                                        <div className="row" style={{maxHeight: '93%'}}>
                                            ...
                                            <div className='col-6' style={{textAlign:'right',paddingTop:'15px'}}>
                                                <span style={{cursor:'pointer'}} onClick={(event) => filtroFecha(3)}>Fecha 
                                                    {seleFecha3 ? (
                                                        <>
                                                            <span style={{marginLeft:'4px'}}>Ascendente</span> 
                                                            <i style={{marginTop:'-4px',color:'red',transform:'rotate(90deg)'}} className='bx bx-transfer-alt'></i>
                                    ...
                                    <div className={`${menuSeleccionado === 4 ? 'div-activo' : ''}`} style={{display:'none'}}>
                                        <div className="row" style={{maxHeight: '93%'}}>
                                            ...
                                            <div className='col-6' style={{textAlign:'right',paddingTop:'15px'}}>
                                                <span style={{cursor:'pointer'}} onClick={(event) => filtroFecha(4)}>Fecha 
                                                    {seleFecha4 ? (
                                                        <>
                                                            <span style={{marginLeft:'4px'}}>Ascendente</span> 
                                                            <i style={{marginTop:'-4px',color:'red',transform:'rotate(90deg)'}} className='bx bx-transfer-alt'></i>
                                    ...
                                    <div className={`${menuSeleccionado === 5 ? 'div-activo' : ''}`} style={{display:'none'}}>
                                        <div className="row" style={{maxHeight: '93%'}}>
                                            ...
                                            <div className='col-6' style={{textAlign:'right',paddingTop:'15px'}}>
                                                <span style={{cursor:'pointer'}} onClick={(event) => filtroFecha(5)}>Fecha 
                                                    {seleFecha5 ? (
                                                        <>
                                                            <span style={{marginLeft:'4px'}}>Ascendente</span> 
                                                            <i style={{marginTop:'-4px',color:'red',transform:'rotate(90deg)'}} className='bx bx-transfer-alt'></i>
                                    ...
                                    <div className={`${menuSeleccionado === 6 ? 'div-activo' : ''}`} style={{display:'none'}}>
                                        <div className="row" style={{maxHeight: '93%'}}>
                                            ...
                                            <div className='col-6' style={{textAlign:'right',paddingTop:'15px'}}>
                                                <span style={{cursor:'pointer'}} onClick={(event) => filtroFecha(6)}>Fecha 
                                                    {seleFecha6 ? (
                                                        <>
                                                            <span style={{marginLeft:'4px'}}>Ascendente</span> 
                                                            <i style={{marginTop:'-4px',color:'red',transform:'rotate(90deg)'}} className='bx bx-transfer-alt'></i>
                                            ...
                                            <div className='col-12' onScroll={handleScrollFiltro(6)} style={{maxHeight: '62vh',overflow:'auto',padding:'0px 7px'}}>
                                ...
                                <div className="col-8" style={{padding:'0px',border: '1px solid #b7b7b7',height: 'calc(100vh - 202px)',maxHeight: 'calc(100vh - 202px)',marginTop: '10px'}}>
                                    {ticketAbierto ? (
                                            ...
                                            <div className='row' style={{width:'100%',margin:'auto'}}>
                                                <div className='col-4' style={{display:'flex'}}>
                                                    ...
                                                    <h7 style={{marginBottom:'0px',marginLeft:'7px',paddingTop:'6px',height:'47px',overflow:'auto',lineHeight:'15px',paddingTop:'10px'}}>
                                                        ...
                                                        <span style={{fontSize:'12px',width:'100%',display:'block'}}>{ticketAbierto.numberLabel}</span>
                                                    </h7>
                                                </div>
                                                <div className='col-8' style={{textAlign: 'right',paddingTop: '10px'}}>
                                                    {ticketAbierto.idStatus == 1 ? (
                                                        ...
                                                        <>
                                                            <div style={{display:'inline-flex'}}>
                                                                <span onClick={(event) => pagoTicket2(ticketAbierto.id)} style={{height:'fit-content',background: '#4ccf2b',color: '#000',border: '2px solid #4ccf2b',borderRadius: '20px',padding: '3px 5px 3px 3px',fontSize: '12px',cursor:'pointer',marginRight:'5px',marginLeft:'5px'}}>
                                                                    <i style={{marginTop:'-1px'}} className='bx bx-dollar'></i>
                                                                    Pago Nuevo Pedido
                                                                </span>
                                                                <span onClick={(event) => reasignarTicket(ticketAbierto)} style={{height:'fit-content',background: '#4eadff',color: '#000',border: '2px solid #4eadff',borderRadius: '20px',padding: '3px 5px 3px 3px',fontSize: '12px',cursor:'pointer',marginRight:'5px',marginLeft:'5px'}}>
                                                        ...
                                                        <>
                                                            <div style={{display:'inline-flex'}}>
                                                                <span onClick={(event) => pagoTicket3(ticketAbierto.id)} style={{height:'fit-content',background: '#d3d3d3',color: '#000',border: '2px solid #d3d3d3',borderRadius: '20px',padding: '3px 5px 3px 3px',fontSize: '12px',cursor:'pointer',marginRight:'5px',marginLeft:'5px'}}>
                                                                    Enviar a cola
                                                                </span>
                                                                <span onClick={(event) => reasignarTicket(ticketAbierto)} style={{height:'fit-content',background: '#4eadff',color: '#000',border: '2px solid #4eadff',borderRadius: '20px',padding: '3px 5px 3px 3px',fontSize: '12px',cursor:'pointer',marginRight:'5px',marginLeft:'5px'}}>
                                                                    <i style={{marginTop:'-1px'}} className='bx bx-transfer'></i>
                                    ...
                                    <div className='row' style={{ display: chatActivo ? 'block' : 'none',background:'#e6e6e6',width:'100%',margin:'auto' }}>
                                        ...
                                        <div className='col-12' style={{paddingTop:'3px',paddingRight:'3px',paddingLeft:'3px'}}>
                                            <div style={{textAlign:'left',marginTop: '0px',background: '#d3d3d3',height: '113px',position: 'absolute',width: '63.8%',overflow: 'auto',zIndex: '10',padding: '10px 5px',display:showDropdown ? 'block' : 'none'}}>
            ...
            <div style={{display: openGaleria ? 'flex' : 'none',background: 'white',position: 'absolute',left: 'calc(100% - 400px)',top: '0px',zIndex: '10000',height: '100%',width: '400px',boxShadow:'0 0 0.375rem 0.25rem rgba(161, 172, 184, 0.15)'}}>
